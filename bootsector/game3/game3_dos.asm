...
DBUFFER_MEMORY_ADR equ 0x3000               ; Doublebuffer memory address
SCREEN_BUFFER_SIZE equ 0xFA00               ; Size of the VGA buffer size

BASE_MEM equ 0x2000                         ; Base memory address
...
restart_game:
    ...
    mov word [PLAYER+1], (0x06)   ; Set player color and sprite   
    shl word [PLAYER+1], 8                  ; Shift left
     or word [PLAYER+1], COLOR_FLY                ; Set player rotation
...
next_level:
    ...
    .next_entitie:
        ...
        mov word [si], COLOR_FLOWER  
        shl word [si], 8
        or word [si], SPRITE_FLOWER
                                             ; Set sprite ID and color for flower
        jmp .spawn_done
        .spawn_spider:
        mov word [si], COLOR_SPIDER
        shl word [si], 8
        or word [si], SPRITE_SPIDER  
...
handle_keyboard:
    ...
    cmp ah, 0x39                            ; Check if Spacebar is pressed
    jne .no_rotate
